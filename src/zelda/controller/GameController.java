...
import zelda.model.GameModel.MusicTrack;
...
public class GameController {
    ...
    private boolean archeryPressed;  // Q
    ...
    private static final int HIT_OFF_X_ACTUAL = (SPRITE_W - HIT_W) / 2; // 8
    private static final int HIT_OFF_Y_ACTUAL = SPRITE_H - HIT_H;      // 22
    ...
    // ---- ARCHERY ANIM ----
    @SuppressWarnings("unused")
    private static final float ARCHERY_DRAW_TIME = 0.25f;
    @SuppressWarnings("unused")
    private static final float ARCHERY_RELEASE_TIME = 0.10f;
    private static final float ARROW_SPEED = 350f;
    private static final int ARROW_DAMAGE = 1;
    @SuppressWarnings("unused")
    private static final int ARROW_W = 8;
    @SuppressWarnings("unused")
    private static final int ARROW_H = 8;
    private static final float ARCHERY_COOLDOWN = 0.5f;

    private float archeryTimer = 0f;
    private float archeryCooldownTimer = 0f;
    private boolean archeryDidHitThisShot = false;

    // ---- ENEMY 1 (Sprite 56x58) ----
    @SuppressWarnings("unused")
    private static final int ENEMY_DRAW_W = 56;
    @SuppressWarnings("unused")
    private static final int ENEMY_DRAW_H = 58;
    private static final int ENEMY_HIT_W = 40;
    private static final int ENEMY_HIT_H = 40;
    private static final int ENEMY_HIT_OFF_X = (ENEMY_DRAW_W - ENEMY_HIT_W) / 2; // 8
    private static final int ENEMY_HIT_OFF_Y = (ENEMY_DRAW_H - ENEMY_HIT_H) / 2; // 9
    ...
    // ---- ENEMY 2 (Sprite 48x48) ----
    @SuppressWarnings("unused")
    private static final int ENEMY2_DRAW_W = 48; 
    @SuppressWarnings("unused")
    private static final int ENEMY2_DRAW_H = 48;
    @SuppressWarnings("unused")
    private static final int ENEMY2_HIT_W = 28;
    @SuppressWarnings("unused")
    private static final int ENEMY2_HIT_H = 28;
    private static final int ENEMY2_HIT_OFF_X = (ENEMY2_DRAW_W - ENEMY2_HIT_W) / 2; // 10
    private static final int ENEMY2_HIT_OFF_Y = (ENEMY2_DRAW_H - ENEMY2_HIT_H) / 2; // 10
    
    @SuppressWarnings("unused")
    private static final float ENEMY2_PATROL_SPEED = 40f;
    @SuppressWarnings("unused")
    private static final float ENEMY2_CHARGE_SPEED = 100f;
    @SuppressWarnings("unused")
    ...
    @SuppressWarnings("unused")
    private static final float ENEMY2_LIFE_DROP_CHANCE = 0.85f;
    ...
    public void pressArchery() { archeryPressed = true; }
    ...
    public void update(float dt) {
        ...
        model.updateBossTimers(dt);
        model.updateArrows(dt);
        
        if (archeryCooldownTimer > 0f) {
            archeryCooldownTimer -= dt;
        }
        ...
        if (archeryPressed && model.getArrows() > 0 && archeryCooldownTimer <= 0f) {
            startArchery();
            updateEnemyForCurrentRoom(dt);
            checkEnemy2TouchDamage();
            checkEnemyTouchDamage();
    ...
    private void updateMusicTrack() {
        ...
        if (model.isTitle()) {
            targetTrack = MusicTrack.TITLE;
        } else if (roomIdx == BOSS_ROOM_INDEX) {
            targetTrack = MusicTrack.BOSS;
        } else {
            targetTrack = MusicTrack.DUNGEON;
        }
        
        model.setMusicTrack(targetTrack);
    }
    ...
    private void clearOneShots() {
        ...
        escPressed = false;
        attackPressed = false;
        archeryPressed = false;
    }

    private void updateEnemyForCurrentRoom(float dt) {
        int roomIdx = model.getCurrentRoomIndex();
        
        if (roomIdx == BOSS_ROOM_INDEX) {
            return;
        }
        
        if (roomIdx == SHOP_INDEX) {
            return;
        }
        ...
        if (roomIdx > 0 && roomIdx != BOSS_ROOM_INDEX) {
            updateEnemy(dt);
        }
    }
    ...
    private void spawnBossBulletTowardsPlayer() {
        ...
        SoundManager.playSound("boss_attack");
    }
    ...
    private void updateBossProjectiles(float dt) {
        ...
        Rectangle playerFeet = new Rectangle(
                Math.round(model.getPlayerX() + HIT_OFF_X_ACTUAL),
                Math.round(model.getPlayerY() + HIT_OFF_Y_ACTUAL),
                HIT_W,
                HIT_H
        );
    ...
    private void updateEnemy(float dt) {
        ...
        int roomIdx = model.getCurrentRoomIndex();
        if (roomIdx == 0 || roomIdx == SHOP_INDEX || roomIdx == BOSS_ROOM_INDEX || roomIdx % 2 != 0) return;
        ...
        if (enemyCollidesWithTiles(ex + ENEMY_HIT_OFF_X, ey + ENEMY_HIT_OFF_Y, ENEMY_HIT_W, ENEMY_HIT_H)) {
            pickNewEnemyDirection();
            model.setEnemyDirTimer(randRange(ENEMY_DIR_MIN, ENEMY_DIR_MAX));
            ex = model.getEnemyX();
            ey = model.getEnemyY();
    ...
    private void updateEnemy2(float dt) {
        int roomIdx = model.getCurrentRoomIndex();
        if (roomIdx == SHOP_INDEX || roomIdx % 2 == 0 || roomIdx == BOSS_ROOM_INDEX) return;
        ...
        for (int i = 0; i < model.getEnemy2MaxUnits(); i++) {
            ...
            if (enemyCollidesWithTiles(nextX + ENEMY2_HIT_OFF_X, nextY + ENEMY2_HIT_OFF_Y, ENEMY2_HIT_W, ENEMY2_HIT_H)) {
                pickNewEnemy2Direction(i);
    ...
    private boolean enemyCollidesWithTiles(float hbX, float hbY, int hbW, int hbH) {
        return collidesAt(hbX, hbY) ||
               collidesAt(hbX + hbW - 1, hbY) ||
               collidesAt(hbX, hbY + hbH - 1) ||
               collidesAt(hbX + hbW - 1, hbY + hbH - 1);
    }
    ...
    private void startAttack() {
        ...
        atkTimer = 0f;
        attackDidHitThisSwing = false;
        SoundManager.playSound("master-sword");
    ...
    private void tryHitEnemyWithSword() {
        ...
        Rectangle enemy = new Rectangle(
                Math.round(model.getEnemyX() + ENEMY_HIT_OFF_X),
                Math.round(model.getEnemyY() + ENEMY_HIT_OFF_Y),
                ENEMY_HIT_W,
                ENEMY_HIT_H
        );
    ...
    private void tryHitEnemy2WithSword() {
        ...
        for (int i = 0; i < model.getEnemy2MaxUnits(); i++) {
            ...
            Rectangle enemy2 = new Rectangle(
                    Math.round(model.getEnemy2X(i) + ENEMY2_HIT_OFF_X),
                    Math.round(model.getEnemy2Y(i) + ENEMY2_HIT_OFF_Y),
                    ENEMY2_HIT_W,
                    ENEMY2_HIT_H
            );
            if (sword.intersects(enemy2)) {
                ...
                if (hpBefore > 0 && !model.isEnemy2Alive(i)) {
                    ...
                    if (Math.random() < ENEMY2_LIFE_DROP_CHANCE) {
                        float potionX = centerX - (POTION_DRAW_W / 2f);
                        float potionY = centerY - (POTION_DRAW_H / 2f);
                        model.spawnPotion(potionX, potionY);
                    }
                }
    ...
    private void tryHitBossWithArrow(int arrowIdx, float x, float y) {
        if (!model.isBossAlive()) return;
        if (model.getCurrentRoomIndex() != BOSS_ROOM_INDEX) return;
    ...
    private void checkRupeePickup() {
        ...
        int roomIdx = model.getCurrentRoomIndex();
        if (roomIdx == SHOP_INDEX || (roomIdx == 0)) return;
    ...
    private void handleShopConfirm() {
        ...
        else if (sel == 1) cost = COST_ARROWS;
        ...
        else if (sel == 1) model.addArrows(10);
    ...
    private void moveWithCollision(float dx, float dy) {
        ...
        float hbX = nextX + HIT_OFF_X_ACTUAL;
        float hbY = nextY + HIT_OFF_Y_ACTUAL;
    ...
    private boolean collidesNpc(float nextX, float nextY) {
        ...
        Rectangle playerRect = new Rectangle(
                Math.round(nextX + HIT_OFF_X_ACTUAL),
                Math.round(nextY + HIT_OFF_Y_ACTUAL),
                HIT_W,
                HIT_H
        );
    ...
    private void checkRoomExit() {
        ...
        float px = model.getPlayerX();
        float py = model.getPlayerY();
        float cx = (px + HIT_OFF_X_ACTUAL) + HIT_W / 2f;
        float cy = (py + HIT_OFF_Y_ACTUAL) + HIT_H / 2f;
        int tileX = (int) (cx / GameModel.TILE_SIZE);
        int tileY = (int) (cy / GameModel.TILE_SIZE);

        float clampXMin = -HIT_OFF_X_ACTUAL;
        float clampXMax = (roomW - HIT_W) - HIT_OFF_X_ACTUAL;
        float clampYMin = -HIT_OFF_Y_ACTUAL;
        float clampYMax = (roomH - HIT_H) - HIT_OFF_Y_ACTUAL;
    ...
    private void startArchery() {
        resetWalkAnim();
        model.setArcheryActive(true);
        model.setArcheryFrame(0);
        archeryTimer = 0f;
        archeryCooldownTimer = ARCHERY_COOLDOWN;
        archeryDidHitThisShot = false;
        model.requestRepaint();
    }

    private void updateArchery(float dt) {
        archeryTimer += dt;
        
        if (archeryTimer < ARCHERY_DRAW_TIME) {
            if (model.getArcheryFrame() != 0) {
                model.setArcheryFrame(0);
                model.requestRepaint();
            }
        } 
        else if (archeryTimer < ARCHERY_DRAW_TIME + ARCHERY_RELEASE_TIME) {
            if (model.getArcheryFrame() != 1) {
                model.setArcheryFrame(1);
                if (archeryTimer - ARCHERY_DRAW_TIME < dt * 1.5f) {
                    spawnArrow();
                }
                model.requestRepaint();
            }
    ...
    private void resetArcheryAnim() {
        model.setArcheryActive(false);
        model.setArcheryFrame(0);
        archeryTimer = 0f;
    }

    private void spawnArrow() {
        if (!archeryDidHitThisShot) {
            archeryDidHitThisShot = true;
            
            float px = model.getPlayerX() + SPRITE_W / 2f;
            float py = model.getPlayerY() + SPRITE_H / 2f;
            
            float vx = 0f, vy = 0f;
            GameModel.Facing facing = model.getFacing();
            
            switch (facing) {
                case UP -> { vy = -ARROW_SPEED; py -= 16; }
                case DOWN -> { vy = ARROW_SPEED; py += 16; }
                case LEFT -> { vx = -ARROW_SPEED; px -= 16; }
                case RIGHT -> { vx = ARROW_SPEED; px += 16; }
            }
            
            model.spawnArrow(px, py, vx, vy, facing);
            SoundManager.playSound("arrow");
        }
    }

    private void updateArrowProjectiles(float dt) {
        for (int i = 0; i < model.getMaxArrows(); i++) {
            if (!model.isArrowAlive(i)) continue;
            
            float x = model.getArrowX(i);
            float y = model.getArrowY(i);
            
            int roomW = Room.COLS * GameModel.TILE_SIZE;
            int roomH = Room.ROWS * GameModel.TILE_SIZE;
            if (x < 0 || y < 0 || x > roomW || y > roomH) {
                model.despawnArrow(i);
                continue;
            }
            ...
            checkArrowHitsEnemy(i, x, y);
            checkArrowHitsEnemy2(i, x, y);
            tryHitBossWithArrow(i, x, y);
        }
    }

    private void checkArrowHitsEnemy(int arrowIdx, float x, float y) {
        if (!model.isEnemyAlive()) return;
        int roomIdx = model.getCurrentRoomIndex();
        if (roomIdx == 0 || roomIdx == SHOP_INDEX || roomIdx == BOSS_ROOM_INDEX || roomIdx % 2 != 0) return;
        ...
        if (arrowRect.intersects(enemy)) {
            model.despawnArrow(arrowIdx);
            SoundManager.playSound("enemy_hit");
            int hpBefore = model.getEnemyHp();
            model.hitEnemy(ARROW_DAMAGE, ENEMY_INVULN_SECONDS, ENEMY_BLINK_SECONDS);
    ...
    private void checkArrowHitsEnemy2(int arrowIdx, float x, float y) {
        int roomIdx = model.getCurrentRoomIndex();
        if (roomIdx == SHOP_INDEX || roomIdx % 2 == 0 || roomIdx == BOSS_ROOM_INDEX) return;
        ...
        for (int i = 0; i < model.getEnemy2MaxUnits(); i++) {
            ...
            if (arrowRect.intersects(enemy2)) {
                SoundManager.playSound("enemy_hit");
                model.despawnArrow(arrowIdx);
                int hpBefore = model.getEnemy2Hp(i);
                model.hitEnemy2(i, ARROW_DAMAGE, ENEMY2_INVULN_SECONDS, ENEMY2_BLINK_SECONDS);
